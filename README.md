...
The plugin is the whole wiring. In Claude Code:
...
- Remove the `autogit hook` entry from `hooks.UserPromptSubmit` in
  `~/.claude/settings.json`, and the `mcp__autogit__commit` and
  `mcp__autogit__branch` entries from `permissions.allow`.
- Delete the `/commit`, `/commit-msg` and `/branch` stubs in
  `~/.claude/commands/` — the plugin's own are namespaced `/autogit:*`.
